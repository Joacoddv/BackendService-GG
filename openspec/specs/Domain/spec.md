# Domain Specification — Pure .NET 8 Domain Layer

**Scope:** `GastroGestion.Domain` — the core domain model for the GastroGestion restaurant system. Defines what MUST be true about the domain's invariants, aggregates, value objects, and behavior.

**Phase:** 2 of 7 in the strangler roadmap.  
**Archive date:** 2026-06-14  
**Status:** COMPLETE — all 23 implementation tasks delivered across three slices (Catalogue, Transactional, Fiscal).  
...
| `Mesa` | — | — |
| `Pedido` | `LineaPedido[]`, `OrdenTrabajo[]` | `Dinero`, `PorcentajeIVA`, `Cantidad`, `DireccionEntrega` (delivery snapshot VO) |
...
`GastroGestion.Domain.csproj` MUST list zero `<PackageReference>` and zero `<ProjectReference>` elements. Verified by REQ-01 below.
...
- `Pedido` is an aggregate root with `Guid Id`, `int NumeroPedido`, `TipoPedido TipoPedido` (Salon / Mostrador / Delivery — TakeAway is treated as Mostrador), `Guid ClienteId`, nullable `Guid MesaId` (required iff `TipoPedido == Salon`), `DireccionEntrega? DireccionEntrega` (required iff `TipoPedido == Delivery`; frozen VO set at creation, never updated), `EstadoPedido Estado`, `EstadoFacturaPedido EstadoFacturacion` (NoFacturado / Facturado / Pagado), `byte[] ConcurrencyToken`, `IReadOnlyList<LineaPedido> Lineas`, `IReadOnlyList<OrdenTrabajo> OrdenesDeTrabajo`.
- `DireccionEntrega` is a value object with the full address fields frozen at creation time.
...
**KNOWN OPEN ITEM (deferred to Phase 3, Application layer):** The spec requires that all `PedidosFacturados` belong to the same `ClienteId`. The domain-layer factory receives only `List<Guid> pedidoIds` — it cannot load the Pedido aggregates to validate ownership. This invariant **cannot be enforced at the domain layer** without crossing the aggregate boundary (architectural violation). The domain provides the `PuedeCombinarseConFactura(Factura otra)` method to check *after* creation; the application layer must validate at factory time by loading the Pedidos and checking their `ClienteId` before calling the factory. Spec scenario 13-G will be enforced as an application-layer invariant in Phase 3. Domain factory does not block this.
...
|---|---|---|
| `PedidoCreado` | `Pedido.Criar` | PedidoId, TipoPedido, ClienteId |
...
## Known Open Items (Deferred to Phase 3)

### Open Item 1: REQ-13/13-G — Multi-client Factura grouping

**Issue:** The spec requires that all `PedidosFacturados` belong to the same `ClienteId`. The domain factory cannot enforce this without cross-aggregate loading, which violates the domain's architectural boundary rule (zero outward dependencies, cannot load repositories).

**Deferral:** This invariant will be enforced by the **Application layer (Phase 3)** before calling the domain factory. The domain provides the `PuedeCombinarseConFactura(Factura otra)` seam for post-creation validation; the application MUST validate at factory time.

**Why:** Maintaining Clean Architecture — the domain layer cannot depend on repositories or other aggregates.

### Open Item 2: IDomainEvent naming — `OccurredOnUtc` vs `OccurredOn`

**Issue:** The spec text (REQ-15) says `DateTime OccurredOn { get; }`. The implementation uses `OccurredOnUtc` across all five domain events.

**Reconciliation:** The implementation is semantically more correct (`OccurredOnUtc` explicitly declares UTC timezone), and consistent across all events. The spec will be updated in Phase 3 to reflect this naming. No behavior change — the field functions identically.
...
**Archived:** 2026-06-14 (Phase 2 of 7 complete)  
